...
- `nuclei` takeover-finding parser is **verified** against real nuclei v3.11.1
  JSONL (real http + dns positives, generated by forcing real templates to match
  controlled input). In-the-wild *detection* of a dangling resource remains
  nuclei's job; a takeover finding is surfaced as an unconfirmed candidate for
  manual review (recon flags, never confirms).
