#!/usr/bin/env python3
"""
Standalone stage 10 (B2a API-schema discovery) runner - re-runs ONLY the
API-schema stage against an existing run directory's assets.db, without
repeating stages 1-9. Mirrors run_content_discovery_only.py's pattern and
IMPORTS the shared run_stage10_and_report from main (never reimplements it -
two implementations drift; see CONTRIBUTING).

Usage:
    python3 run_stage10_only.py --run-dir /path/to/existing_run_directory

Expects a populated assets.db with at least one confirmed-live in_scope host
(carrying httpx_status_code, i.e. stage 4 has already run). Requires httpx on
PATH (and pyyaml for YAML specs).
"""
...
from main import run_stage10_and_report
...
def main():
    parser = argparse.ArgumentParser(description="Re-run stage 10 (API-schema discovery) only")
    ...
    logger.info("Found %d confirmed-live in_scope host(s) - proceeding with stage 10 only",
                len(live_hosts))

    run_stage10_and_report(patterns, state, scope)

    logger.info("Stage 10 complete. See %s", state.assets_db_path)
